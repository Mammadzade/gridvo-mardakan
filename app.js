...
const ICONS={
  ...
  receipt:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 3h15v13H1z"/><path d="M16 8h4l3 5v3h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/><path d="m4 8-3 0"/></svg>',
  shipment:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13"/><path d="M16 8h4l3 5v3h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/><path d="m20 8 3 0"/></svg>',
  ...
  move:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 16l-4-4 4-4"/><path d="M3 12h18"/><path d="M17 8l4 4-4 4"/></svg>',
...
async function logout(){await sb.auth.signOut();localStorage.removeItem('gridvo_role');document.getElementById('auth-screen').style.display='flex';document.getElementById('app').style.display='none';currentUser=null;}

async function initUser(user){currentUser=user;const savedRole=localStorage.getItem('gridvo_role');const userRole=user.user_metadata?.role||'warehouse';currentRole=(savedRole&&(userRole==='admin'||savedRole===userRole))?savedRole:userRole;updateRoleUI();document.getElementById('auth-screen').style.display='none';document.getElementById('app').style.display='block';await buildSidebar();navigateTo(currentRole==='marketing'?'new_receipt_req':'tasks');}
...
async function switchRole(role){currentRole=role;localStorage.setItem('gridvo_role',role);updateRoleUI();await buildSidebar();closeModal();navigateTo(role==='marketing'?'new_receipt_req':role==='warehouse'?'tasks':'dashboard');toast('Роль: '+(role==='admin'?'Администратор':role==='warehouse'?'Склад':'Маркетинг'));}
...
const counts={receipt:0,shipment:0,writeoff:0,placement:0,tasks:0};
(newTasks||[]).forEach(function(t){if(t.type==='receipt')counts.receipt++;if(t.type==='shipment')counts.shipment++;if(t.type==='writeoff')counts.writeoff++;if(t.type==='placement')counts.placement++;counts.tasks++;});
const menus={
  admin:[
    {sec:'Обзор'},
    ...
    {id:'receipt',label:'Приёмка',color:'linear-gradient(135deg,#00D26A,#00A854)',badge:counts.receipt},
    {id:'placement',label:'Размещение',color:'linear-gradient(135deg,#F59E0B,#D97706)',badge:counts.placement},
    ...
    {id:'requests',label:'Мои заявки',color:'linear-gradient(135deg,#00D26A,#00A854)'},
  ],
  warehouse:[
    {sec:'Задачи'},
  ...
  marketing:[
    {sec:'Мои заявки'},
    {id:'requests',label:'Все заявки',color:'linear-gradient(135deg,#00D26A,#00A854)'},
    {sec:'Создать заявку'},
    {id:'new_receipt_req',label:'Заявка на приёмку',color:'linear-gradient(135deg,#00D26A,#00A854)'},
...
items.forEach(function(item){
  ...
  html+='<div class="nav-icon" style="background:'+item.color+'"><svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" width="20" height="20">'+ICONS[iconKey]+'</svg></div>';
...
const{data:products}=await sb.from('products').select('*,categories(name),cells(label),product_photos(url)').eq('status','На складе').order('created_at',{ascending:false});
const canEdit=currentRole!=='marketing';
const isAdmin=currentUser.user_metadata?.role==='admin';
const placed=(products||[]).filter(function(p){return p.cell_id;});
const unplaced=(products||[]).filter(function(p){return!p.cell_id;});
const activeTab=window._prodTab||'unplaced';
...
html+='<button class="btn '+(activeTab==='unplaced'?'btn-primary':'btn-secondary')+'" onclick="window._prodTab=&quot;unplaced&quot;;renderPage(&quot;products&quot;)">📦 Не размещённые ('+unplaced.length+')</button>';
html+='<button class="btn '+(activeTab==='placed'?'btn-primary':'btn-secondary')+'" onclick="window._prodTab=&quot;placed&quot;;renderPage(&quot;products&quot;)">✅ Размещённые ('+placed.length+')</button>';
html+='</div>';
const showProducts=activeTab==='placed'?placed:unplaced;
...
(showProducts||[]).forEach(function(p){
...
const{data:p}=await sb.from('products').select('*,categories(name),cells(label),product_photos(url)').eq('id',id).single();
if(!p)return;
const isAdmin=currentUser.user_metadata?.role==='admin';
const canEdit=currentRole!=='marketing';
let html='<div class="modal-title">'+p.name+'</div>';
if(p.product_photos&&p.product_photos.length)html+='<img src="'+p.product_photos[0].url+'" style="width:100%;height:200px;object-fit:cover;border-radius:10px;margin-bottom:16px"/>';
html+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px;font-size:13px">';
html+='<div><span style="color:var(--mid)">Артикул:</span> <strong>'+p.article+'</strong></div>';
html+='<div><span style="color:var(--mid)">Статус:</span> <span class="badge '+(p.status==='На складе'?'badge-green':'badge-gray')+'">'+p.status+'</span></div>';
html+='<div><span style="color:var(--mid)">Категория:</span> '+((p.categories&&p.categories.name)?p.categories.name:'—')+'</div>';
html+='<div><span style="color:var(--mid)">Ячейка:</span> '+((p.cells&&p.cells.label)?'<span class="badge badge-amber">'+p.cells.label+'</span>':'—')+'</div>';
html+='<div><span style="color:var(--mid)">Кол-во:</span> '+p.quantity+'</div>';
html+='<div><span style="color:var(--mid)">Добавлен:</span> '+fmtDate(p.created_at)+'</div></div>';
if(p.description)html+='<div class="info-box">'+p.description+'</div>';
if(canEdit){
  html+='<div class="form-row" style="margin-top:14px"><label class="label">📸 Добавить фото</label>';
  html+='<input type="file" id="dphoto" accept="image/*" class="input" style="padding:7px"/></div>';
  html+='<button class="btn btn-secondary btn-sm" style="margin-bottom:14px" onclick="uploadDetailPhoto(this.dataset.pid)" data-pid="'+p.id+'">Загрузить фото</button>';
}
html+='<div class="modal-footer">';
if(isAdmin)html+='<button class="btn btn-danger" data-pid="'+p.id+'" onclick="deleteProduct(this.dataset.pid);closeModal()">🗑️ Удалить</button>';
html+='<button class="btn btn-secondary" onclick="closeModal()">Закрыть</button></div>';
openModal(html);}

async function uploadDetailPhoto(prodId){
const input=document.getElementById('dphoto');
if(!input||!input.files[0]){toast('Выберите файл','error');return;}
...
if(json.secure_url){
  await sb.from('product_photos').delete().eq('product_id',prodId);
  await sb.from('product_photos').insert({product_id:prodId,url:json.secure_url});
...
async function exportExcel(){try{const{data:products}=await sb.from('products').select('*,categories(name),cells(label)').order('created_at',{ascending:false});if(!products||!products.length){toast('Нет данных для экспорта','error');return;}const rows=products.map(function(p){return{'Наименование':p.name||'','Артикул':p.article||'','Категория':(p.categories&&p.categories.name)||'','Количество':p.quantity||0,'Ячейка':(p.cells&&p.cells.label)||'','Статус':p.status||'','Состояние':p.condition||'','Описание':p.description||'','Добавлен':fmtDate(p.created_at)};});const ws=XLSX.utils.json_to_sheet(rows);const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,'Остатки',ws);XLSX.writeFile(wb,'Gridvo_Остатки_'+new Date().toLocaleDateString('ru-RU').replace(/[.]/g,'-')+'.xlsx');toast('Excel скачан!');}catch(e){toast('Ошибка экспорта: '+e.message,'error');console.error(e);}}
...
const placementReqNum=genReqNum();
const{data:placementTask}=await sb.from('tasks').insert({type:'placement',status:'Новая',request_number:placementReqNum,created_by_name:'Система (после приёмки '+task.request_number+')',assigned_to:'warehouse',note:'Разместить товары из приёмки '+task.request_number}).select().single();
if(placementTask&&prodIds.length){
  for(let i=0;i<prodIds.length;i++){
    await sb.from('task_items').insert({task_id:placementTask.id,product_id:prodIds[i],quantity_required:1,quantity_collected:0,is_collected:false});
  }
}
toast('Приёмка завершена! Задача на размещение создана 📍','success');
closeModal();await buildSidebar();renderPage('placement');}
async function renderPlacement(c){
const{data:tasks}=await sb.from('tasks').select('*,task_items(*,products(name,article))').eq('type','placement').order('created_at',{ascending:false});
const newCount=(tasks||[]).filter(function(t){return t.status==='Новая';}).length;
const isAdmin=currentUser.user_metadata?.role==='admin';
const sC={'Новая':'badge-amber','В работе':'badge-blue','Выполнена':'badge-green'};
let html='<div class="page-title">Размещение';
if(newCount>0)html+=' <span class="badge badge-red">'+newCount+' новых</span>';
html+='<button class="btn btn-primary btn-sm" onclick="openPlacementForm()">+ Разместить вручную</button></div>';
if(tasks&&tasks.length){
  tasks.forEach(function(t){
    const tid=t.id;
    html+='<div class="task-card '+(t.status==='Выполнена'?'done':t.status==='Новая'?'new':'')+'" data-tid="'+tid+'" onclick="openPlacementTask(this.dataset.tid)">';
    html+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">';
    html+='<div><div style="font-weight:700;font-size:14px">Размещение — '+t.request_number+'</div>';
    html+='<div style="font-size:12px;color:var(--mid)">'+(t.note||'—')+' · '+fmtDT(t.created_at)+'</div></div>';
    html+='<div style="display:flex;gap:6px;align-items:center">';
    html+='<span class="badge '+(sC[t.status]||'badge-gray')+'">'+t.status+'</span>';
    if(isAdmin)html+='<button class="btn btn-sm btn-danger" data-tid2="'+tid+'" onclick="event.stopPropagation();deleteTask(this.dataset.tid2,"placement")">🗑️</button>';
    html+='</div></div>';
    if(t.task_items&&t.task_items.length)html+='<div style="font-size:12px;color:var(--mid)">Товаров: '+t.task_items.length+'</div>';
    html+='</div>';
  });
}else{html+='<div class="card" style="text-align:center;color:var(--mid);padding:40px">Задач на размещение нет</div>';}
c.innerHTML=html;}

async function openPlacementTask(id){
const{data:task}=await sb.from('tasks').select('*,task_items(*,products(name,article,cells(label)))').eq('id',id).single();
if(!task)return;
window._currentTaskId=id;
const isAdmin=currentUser.user_metadata?.role==='admin';
let html='<div class="modal-title">📍 Размещение — '+task.request_number+'</div>';
html+='<div class="info-box">'+task.note+'</div>';
html+='<div style="margin-bottom:14px">';
(task.task_items||[]).forEach(function(item){
html+='<div style="display:flex;align-items:center;gap:10px;padding:10px;background:#E8F5EF;border-radius:8px;margin-bottom:6px">';
html+='<span style="font-size:18px">'+(item.is_collected?'✅':'⬜')+'</span>';
html+='<div style="flex:1"><div style="font-weight:600;font-size:13px">'+(item.products&&item.products.name?item.products.name:'—')+'</div>';
html+='<div style="font-size:12px;color:var(--mid)">'+(item.products&&item.products.article?item.products.article:'—')+'</div></div>';
html+='<div style="font-size:12px;color:var(--mid)">'+(item.products&&item.products.cells&&item.products.cells.label?'<span class="badge badge-amber">'+item.products.cells.label+'</span>':'Не размещён')+'</div></div>';
});
html+='</div><div class="modal-footer">';
if(task.status!=='Выполнена')html+='<button class="btn btn-success" onclick="completePlacementTask(window._currentTaskId)">✓ Завершить размещение</button>';
if(isAdmin)html+='<button class="btn btn-danger" onclick="deleteTask(window._currentTaskId,"placement");closeModal()">🗑️ Удалить</button>';
html+='<button class="btn btn-secondary" onclick="closeModal()">Закрыть</button></div>';
openModal(html);}

async function completePlacementTask(id){
await sb.from('tasks').update({status:'Выполнена',completed_at:new Date().toISOString(),executed_by_name:currentUser.user_metadata?.name}).eq('id',id);
toast('Размещение завершено!');closeModal();await buildSidebar();renderPage('placement');}
